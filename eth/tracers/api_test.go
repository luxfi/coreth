...
func newTestBackend(t *testing.T, n int, gspec *core.Genesis, scheme string, generator func(i int, b *core.BlockGen)) *testBackend {
	...
	if scheme == customrawdb.DatabaseScheme {
		cacheConfig.SnapshotLimit = 0 // Database does not support snapshots
	}
...
func testTraceCall(t *testing.T, scheme string) {
	...
	genesis := &core.Genesis{
		Config: params.TestChainConfig, // TODO: go-ethereum has not enabled Shanghai yet, so we use Banff here so tests pass.
...
func testTracingWithOverrides(t *testing.T, scheme string) {
	...
	genesis := &core.Genesis{
		Config: params.TestChainConfig, // TODO: go-ethereum has not enabled Shanghai yet, so we use Cortina here so tests pass.
		Alloc: types.GenesisAlloc{
			accounts[0].addr: {Balance: big.NewInt(params.Ether)},
			accounts[1].addr: {Balance: big.NewInt(params.Ether)},
			accounts[2].addr: {Balance: big.NewInt(params.Ether)},

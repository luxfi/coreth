...
all:
	...
	go build -o $(GOBIN)/devp2p ./cmd/devp2p
	go build -o $(GOBIN)/ethkey ./cmd/ethkey
...
lint: ## Run linters.
	@echo "Linting requires golangci-lint. Install with: go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest"
	golangci-lint run

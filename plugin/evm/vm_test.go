...
func newVM(t *testing.T, config testVMConfig) *testVM {
	...
	for addr, luxAmount := range config.utxos {
		...
		if _, err := addUTXO(atomicMemory, innerVM.ctx, txID, 0, innerVM.ctx.UTXOAssetID, luxAmount, addr); err != nil {
...
// Database cannot yet be run with an empty config.
func getConfig(scheme, otherConfig string) string {
	innerConfig := otherConfig
	if scheme == customrawdb.DatabaseScheme {
		if len(innerConfig) > 0 {
			innerConfig += ", "
		}
		innerConfig += fmt.Sprintf(`"state-scheme": "%s", "snapshot-cache": 0, "pruning-enabled": true, "state-sync-enabled": false, "metrics-expensive-enabled": false`, customrawdb.DatabaseScheme)
	}
...
func (vm *testVM) WaitForEvent(ctx context.Context) commonEng.Message {
	...
	msg, ok := result.(commonEng.Message)
...
func addUTXO(sharedMemory *luxatomic.Memory, ctx *consensusctx.Context, txID ids.ID, index uint32, assetID ids.ID, amount uint64, addr ids.ShortID) (*lux.UTXO, error) {
	utxo := &lux.UTXO{
		UTXOID: lux.UTXOID{
			TxID:        txID,
			OutputIndex: index,
...
func testIssueAtomicTxs(t *testing.T, scheme string) {
	...
	exportTx, err := tvm.atomicVM.NewExportTx(tvm.vm.ctx.UTXOAssetID, importAmount-(2*genesisparams.AtomicTxFee), tvm.vm.ctx.XChainID, testShortIDAddrs[0], initialBaseFee, []*secp256k1.PrivateKey{testKeys[0]})
	if err != nil {
		t.Fatal(err)
	}
...
func testBuildEthTxBlock(t *testing.T, scheme string) {
	...
	newCTX := consensustest.Context(t, consensustest.CChainID)
...
func testReissueAtomicTxHigherGasPrice(t *testing.T, scheme string) {
	...
	tests := map[string]func(t *testing.T, vm *atomicvm.VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, discarded []*atomic.Tx){
		"single UTXO override": func(t *testing.T, vm *atomicvm.VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, evicted []*atomic.Tx) {
			utxo, err := addUTXO(sharedMemory, vm.Ctx, ids.GenerateTestID(), 0, vm.Ctx.UTXOAssetID, constants.Lux, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
		...
		"one of two UTXOs overrides": func(t *testing.T, vm *atomicvm.VM, sharedMemory *luxatomic.Memory) (issued []*atomic.Tx, evicted []*atomic.Tx) {
			utxo1, err := addUTXO(sharedMemory, vm.Ctx, ids.GenerateTestID(), 0, vm.Ctx.UTXOAssetID, constants.Lux, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
			utxo2, err := addUTXO(sharedMemory, vm.Ctx, ids.GenerateTestID(), 0, vm.Ctx.UTXOAssetID, constants.Lux, testShortIDAddrs[0])
			if err != nil {
				t.Fatal(err)
			}
...
func testBonusBlocksTxs(t *testing.T, scheme string) {
	...
	utxo := &lux.UTXO{
		UTXOID: utxoID,
		Asset:  lux.Asset{ID: tvm.vm.ctx.UTXOAssetID},
		Out: &secp256k1fx.TransferOutput{
			Amt: importAmount,
			OutputOwners: secp256k1fx.OutputOwners{
				Threshold: 1,
...
func testBuildInvalidBlockHead(t *testing.T, scheme string) {
	...
	utx := &atomic.UnsignedImportTx{
		...
		Outs: []atomic.EVMOutput{{
			...
			AssetID: tvm.vm.ctx.UTXOAssetID,
		}},
...
func TestBuildBlockDoesNotExceedAtomicGasLimit(t *testing.T) {
	...
	for i := 0; i < mempoolTxs; i++ {
		utxo, err := addUTXO(tvm.atomicMemory, tvm.vm.ctx, txID, uint32(i), tvm.vm.ctx.UTXOAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)
...
func TestExtraStateChangeAtomicGasLimitExceeded(t *testing.T) {
	...
	// in ApricotPhase5.
	for i := 0; i < 100; i++ {
		_, err := addUTXO(tvm1.atomicMemory, tvm1.vm.ctx, txID, uint32(i), tvm1.vm.ctx.UTXOAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)

		_, err = addUTXO(tvm2.atomicMemory, tvm2.vm.ctx, txID, uint32(i), tvm2.vm.ctx.UTXOAssetID, importAmount, testShortIDAddrs[0])
		assert.NoError(t, err)
	}
...
func TestSkipChainConfigCheckCompatible(t *testing.T) {
	...
	newCTX := consensustest.Context(t, tvm.vm.ctx.ChainID)
...
func TestBuildBlockWithInsufficientCapacity(t *testing.T) {
	...
	importAmount := uint64(2_000_000_000_000_000) // 2M LUX
...
func TestWaitForEvent(t *testing.T) {
	populateAtomicMemory := func(t *testing.T, tvm *testVM) (common.Address, *ecdsa.PrivateKey) {
		...
		xChainSharedMemory := tvm.atomicMemory.NewSharedMemory(tvm.vm.ctx.XChainID)
		inputID := utxo.InputID()
		require.NoError(t, xChainSharedMemory.Apply(map[ids.ID]*luxatomic.Requests{tvm.vm.ctx.ChainID: {PutRequests: []*luxatomic.Element{{
			Key:   inputID[:],
			Value: utxoBytes,
			Traits: [][]byte{
				testKeys[0].Address().Bytes(),

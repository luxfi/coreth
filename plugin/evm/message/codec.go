...
func marshalValue(source interface{}, p *wrappers.Packer) error {
	...
	case *CodeResponse:
		marshalCodeResponse(*v, p)
	case BlockSyncSummary:
		marshalBlockSyncSummary(v, p)
	case *BlockSyncSummary:
		marshalBlockSyncSummary(*v, p)
	case *Request:
...
func unmarshalLeafsRequest(l *LeafsRequest, p *wrappers.Packer) {
	...
	l.NodeType = NodeType(p.UnpackByte())
}
...
func marshalBlockSyncSummary(s BlockSyncSummary, p *wrappers.Packer) {
	p.PackLong(s.BlockNumber)
	p.PackFixedBytes(s.BlockHash[:])
	p.PackFixedBytes(s.BlockRoot[:])
}

func unmarshalBlockSyncSummary(s *BlockSyncSummary, p *wrappers.Packer) {

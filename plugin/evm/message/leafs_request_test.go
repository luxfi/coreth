...
func TestMarshalLeafsRequest(t *testing.T) {
	...
	_, err := rng.Read(startBytes)
	assert.NoError(t, err)

	_, err = rng.Read(endBytes)
	assert.NoError(t, err)
...
func TestMarshalLeafsResponse(t *testing.T) {
	...
	for i := range keysBytes {
		...
		valsBytes[i] = make([]byte, rng.Intn(8)+8) // min 8 bytes, max 16 bytes

		_, err := rng.Read(keysBytes[i])
		assert.NoError(t, err)
		_, err = rng.Read(valsBytes[i])
		assert.NoError(t, err)
	}

	nextKey := make([]byte, common.HashLength)
	_, err := rng.Read(nextKey)
	assert.NoError(t, err)

	proofVals := make([][]byte, 4)
	for i := range proofVals {
		proofVals[i] = make([]byte, rng.Intn(8)+8) // min 8 bytes, max 16 bytes

		_, err = rng.Read(proofVals[i])
		assert.NoError(t, err)
	}

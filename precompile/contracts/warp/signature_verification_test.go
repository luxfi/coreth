...
type signatureTest struct {
	...
	quorumNum    uint64
	quorumDen    uint64
	msgF         func(*require.Assertions) *warp.Envelope
	verifyErr    error
	canonicalErr error
}
...
func TestSignatureVerification(t *testing.T) {
	tests := []signatureTest{
		{
			name: "can't get chainID",
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validatorsmock.NewState(ctrl)
				state.EXPECT().GetChainID(gomock.Any(), sourceChainID).Return(sourceChainID, errTest)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
			msgF: func(require *require.Assertions) *warp.Envelope {
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validatorsmock.NewState(ctrl)
				state.EXPECT().GetChainID(gomock.Any(), sourceChainID).Return(sourceChainID, nil)
				state.EXPECT().GetValidatorSet(gomock.Any(), pChainHeight, sourceChainID).Return(nil, errTest)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				state := validatorsmock.NewState(ctrl)
				state.EXPECT().GetChainID(gomock.Any(), sourceChainID).Return(sourceChainID, nil)
				state.EXPECT().GetValidatorSet(gomock.Any(), pChainHeight, sourceChainID).Return(vdrs, nil)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,

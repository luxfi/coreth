...
func TestGetVerifiedWarpMessage(t *testing.T) {
	...
	require.NoError(t, err)
	warpMessage, err := warp.NewEnvelope(unsignedWarpMsg, &warp.BitSetSignature{}) // Create message with empty signature for testing
	require.NoError(t, err)
	...
	tests := map[string]precompiletest.PrecompileTest{
		...
		"get message invalid addressed payload": {
			...
			Predicates: func() [][]byte {
				...
				require.NoError(t, err)
				warpMessage, err := warp.NewEnvelope(unsignedMessage, &warp.BitSetSignature{})
				require.NoError(t, err)

...
var (
	...
	ffiProposeCount         = metrics.GetOrRegisterCounter("database/triedb/propose/count", nil)
	ffiProposeTimer         = metrics.GetOrRegisterCounter("database/triedb/propose/time", nil)
	ffiCommitCount          = metrics.GetOrRegisterCounter("database/triedb/commit/count", nil)
	ffiCommitTimer          = metrics.GetOrRegisterCounter("database/triedb/commit/time", nil)
	ffiCleanupTimer         = metrics.GetOrRegisterCounter("database/triedb/cleanup/time", nil)
	ffiOutstandingProposals = metrics.GetOrRegisterGauge("database/triedb/propose/outstanding", nil)

	// FFI Trie operation metrics
	ffiHashCount = metrics.GetOrRegisterCounter("database/triedb/hash/count", nil)
	ffiHashTimer = metrics.GetOrRegisterCounter("database/triedb/hash/time", nil)
	ffiReadCount = metrics.GetOrRegisterCounter("database/triedb/read/count", nil)
	ffiReadTimer = metrics.GetOrRegisterCounter("database/triedb/read/time", nil)
)
...
// ProposalContext represents a proposal in the Database database.
...
type Database struct {
	fwDisk       *ffi.Database // The underlying Database database, used for storing proposals and revisions.
...
// New creates a new Database database with the given disk database and configuration.
...
func New(config *Config) *Database {
	...
	if err != nil {
		log.Crit("database: error validating config", "error", err)
	}

	fw, err := ffi.New(config.FilePath, fwConfig)
	if err != nil {
		log.Crit("database: error creating database database", "error", err)
	}

	currentRoot, err := fw.Root()
	if err != nil {
		log.Crit("database: error getting current root", "error", err)
	}
...
func validatePath(trieConfig *Config) (*ffi.Config, error) {
	if trieConfig.FilePath == "" {
		return nil, fmt.Errorf("database database file path must be set")
	}
	...
	// Create the Database config from the provided config.
	config := &ffi.Config{
...
// to recognize the Database database.
...
func (db *Database) Initialized(_ common.Hash) bool {
	...
	if err != nil {
		log.Error("database: error getting current root", "error", err)
		return false
	}
...
func (db *Database) Update(root common.Hash, parentRoot common.Hash, block uint64, nodes *trienode.MergedNodeSet, _ *triestate.Set, opts ...stateconf.TrieDBUpdateOption) error {
	...
	if !ok {
		log.Error("database: no block hash provided for block %d", block)
	}
	...
	if existingProposals, ok := db.proposalMap[root]; ok {
		for _, existing := range existingProposals {
			...
			if _, exists := existing.Hashes[hash]; exists {
				log.Debug("database: proposal already exists", "root", root.Hex(), "parent", parentRoot.Hex(), "block", block, "hash", hash.Hex())
				return nil
			}
			...
			if _, exists := existing.Parent.Hashes[parentHash]; exists {
				log.Debug("database: proposal already exists, updating hash", "root", root.Hex(), "parent", parentRoot.Hex(), "block", block, "hash", hash.Hex())
...
func (db *Database) propose(root common.Hash, parentRoot common.Hash, hash common.Hash, parentHash common.Hash, block uint64, keys [][]byte, values [][]byte) error {
	...
	for _, parentProposal := range db.proposalMap[parentRoot] {
		...
		log.Debug("database: proposing from parent proposal", "parent", parentProposal.Root.Hex(), "root", root.Hex(), "height", block)
	...
	if _, exists := db.proposalTree.Hashes[parentHash]; db.proposalTree.Block != 0 && !exists {
		return fmt.Errorf("database: parent hash %s not found for block %s at height %d", parentHash.Hex(), hash.Hex(), block)
	} else if db.proposalTree.Root != parentRoot {
		return fmt.Errorf("database: parent root %s does not match proposal tree root %s for root %s at height %d", parentRoot.Hex(), db.proposalTree.Root.Hex(), root.Hex(), block)
	}

	log.Debug("database: proposing from database root", "root", root.Hex(), "height", block)
...
func (db *Database) Commit(root common.Hash, report bool) (err error) {
	...
	for _, possible := range db.proposalMap[root] {
		if possible.Parent.Root == db.proposalTree.Root && possible.Parent.Block == db.proposalTree.Block {
			...
			if pCtx != nil {
				...
				return fmt.Errorf("database: multiple proposals found for %s", root.Hex())
			}
			pCtx = possible
		}
	}
	if pCtx == nil {
		return fmt.Errorf("database: committable proposal not found for %s", root.Hex())
	}

	start := time.Now()
	// Commit the proposal to the database.
	if commitErr := pCtx.Proposal.Commit(); commitErr != nil {
		return fmt.Errorf("database: error committing proposal %s: %w", root.Hex(), commitErr)
	}
	...
	if err != nil {
		return fmt.Errorf("database: error getting current root after commit: %w", err)
	}
	currentRoot := common.BytesToHash(currentRootBytes)
	if currentRoot != root {
		return fmt.Errorf("database: current root %s does not match expected root %s", currentRoot.Hex(), root.Hex())
	}

	if report {
		log.Info("Persisted proposal to database database", "root", root)
	} else {
		log.Debug("Persisted proposal to database database", "root", root)
	}
	return nil
}
...
// This will be implemented in the database database eventually.
// Currently, Database stores all revisions in disk and proposals in memory.
...
func (db *Database) Reference(_ common.Hash, _ common.Hash) {
	log.Error("database: Reference not implemented")
}
...
func (db *Database) Close() error {
	...
	// within the database close method.
	db.proposalMap = nil
...
func (db *Database) createProposal(layer proposable, root common.Hash, keys, values [][]byte) (p *ffi.Proposal, err error) {
	...
	defer func() {
		if err != nil && p != nil {
			if dropErr := p.Drop(); dropErr != nil {
				...
				log.Error("database: error dropping proposal after error", "root", root.Hex(), "error", dropErr)
			}
			p = nil
		}
	}()
	...
	if len(keys) != len(values) {
		return nil, fmt.Errorf("database: keys and values must have the same length, got %d keys and %d values", len(keys), len(values))
	}

	start := time.Now()
	p, err = layer.Propose(keys, values)
	if err != nil {
		return nil, fmt.Errorf("database: unable to create proposal for root %s: %w", root.Hex(), err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("database: error getting root of proposal %s: %w", root, err)
	}
	currentRoot := common.BytesToHash(currentRootBytes)
	if root != currentRoot {
		return nil, fmt.Errorf("database: proposed root %s does not match expected root %s", currentRoot.Hex(), root.Hex())
	}
...
func (db *Database) dereference(pCtx *ProposalContext) {
	...
	if err := pCtx.Proposal.Drop(); err != nil {
		log.Error("database: error dropping proposal", "root", pCtx.Root.Hex(), "error", err)
	}
	ffiOutstandingProposals.Dec(1)
}
...
func (db *Database) Reader(root common.Hash) (database.Reader, error) {
	if _, err := db.fwDisk.GetFromRoot(root.Bytes(), []byte{}); err != nil {
		return nil, fmt.Errorf("database: unable to retrieve from root %s: %w", root.Hex(), err)
	}
...
func (reader *reader) Node(_ common.Hash, path []byte, _ common.Hash) ([]byte, error) {
	// TODO: remove these locks once Database supports concurrent reads and commits.
	reader.db.proposalLock.RLock()
	defer reader.db.proposalLock.RUnlock()
	// This function relies on Database's internal locking to ensure concurrent reads are safe.
...
func (db *Database) getProposalHash(parentRoot common.Hash, keys, values [][]byte) (common.Hash, error) {
	...
	if db.proposalTree.Root == parentRoot {
		...
		if err != nil {
			return common.Hash{}, fmt.Errorf("database: error proposing from root %s: %v", parentRoot.Hex(), err)
		}
	} else {
		...
		if !ok || len(proposals) == 0 {
			return common.Hash{}, fmt.Errorf("database: no proposal found for parent root %s", parentRoot.Hex())
		}
		rootProposal := proposals[0].Proposal

		p, err = rootProposal.Propose(keys, values)
		if err != nil {
			return common.Hash{}, fmt.Errorf("database: error proposing from parent proposal %s: %v", parentRoot.Hex(), err)
		}
	}
	ffiHashCount.Inc(1)

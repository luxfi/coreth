...
//  2. The `Hash` method actually creates the proposal, since Database cannot calculate
...
func (a *AccountTrie) NodeIterator(_ []byte) (trie.NodeIterator, error) {
	return nil, errors.New("NodeIterator not implemented for Database")
}

// Prove implements state.Trie.
func (a *AccountTrie) Prove(_ []byte, _ ethdb.KeyValueWriter) error {
	return errors.New("Prove not implemented for Database")
}

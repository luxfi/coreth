...
import (
	...
	"github.com/luxfi/geth/ethdb/zapdb"
...
func testRepair(t *testing.T, tt *rewindTest, snapshots bool) {
	for _, scheme := range []string{rawdb.HashScheme, rawdb.PathScheme, customrawdb.DatabaseScheme} {
...
func testRepairWithScheme(t *testing.T, tt *rewindTest, snapshots bool, scheme string) {
	...
	if scheme == customrawdb.DatabaseScheme && snapshots {
		t.Skip("Database scheme does not support snapshots")
	}

	// Create a temporary persistent database
	datadir := t.TempDir()

	kvdb, err := zapdb.New(datadir, 0, 0, "", false)
	if err != nil {
	...
	if tt.sidechainBlocks > 0 {
		...
		sideblocks, _, err = GenerateChain(gspec.Config, gspec.ToBlock(), engine, genDb, tt.sidechainBlocks, 10, func(i int, b *BlockGen) {
			...
			tx, err := types.SignTx(types.NewTransaction(b.TxNonce(addr1), common.Address{0x01}, big.NewInt(10000), params.TxGas, big.NewInt(dynamicfee.InitialBaseFee), nil), signer, key1)
			require.NoError(err)
			b.AddTx(tx)
		})
		require.NoError(err)
	...
	canonblocks, _, err := GenerateChain(gspec.Config, gspec.ToBlock(), engine, genDb, tt.canonicalBlocks, 10, func(i int, b *BlockGen) {
		...
		tx, err := types.SignTx(types.NewTransaction(b.TxNonce(addr1), common.Address{0x02}, big.NewInt(10000), params.TxGas, big.NewInt(dynamicfee.InitialBaseFee), nil), signer, key1)
		require.NoError(err)
		b.AddTx(tx)
	})
	require.NoError(err)

...
func ReadChainConfig(db ethdb.KeyValueReader, hash common.Hash) *gethparams.ChainConfig {
	...
	// TODO: Handle upgrade config for evm-specific fields
	// For now, just return the base config
	return config
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db ethdb.KeyValueWriter, hash common.Hash, config *gethparams.ChainConfig) {
	ethrawdb.WriteChainConfig(db, hash, config)
	if config == nil {
		return
	}

	// TODO: Handle evm-specific upgrade config
	// For now, skip writing upgrade config
}

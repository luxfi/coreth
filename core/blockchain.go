...
type CacheConfig struct {
	...
	ChainDataDir    string // Directory to store chain data in (used by Database)
...
func DefaultCacheConfigWithScheme(scheme string) *CacheConfig {
	...
	// TODO: remove this once if Database supports snapshots
	if config.StateScheme == customrawdb.DatabaseScheme {
		config.SnapshotLimit = 0 // no snapshot allowed for database
	}
	return &config
}
...
func (bc *BlockChain) reprocessState(current *types.Block, reexec uint64) error {
	...
	// Database requires processing each root individually.
	if bc.CacheConfig().StateScheme == customrawdb.DatabaseScheme {

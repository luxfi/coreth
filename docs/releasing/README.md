...
1. Because Luxd and coreth depend on each other, and that we create releases of Luxd before coreth, you can use a recent commit hash or recent release candidate of Luxd in your `go.mod` file. Coreth releases should be tightly coordinated with Luxd releases.
...
1. Create a pull request (PR) from your branch targeting master, for example using [`gh`](https://cli.github.com/):
    ...
    ```bash
    gh pr create --repo github.com/luxfi/subnet-evm --base master --title "chore: release $VERSION_RC"
    ```
...
1. Once the release candidate tag is created, create a pull request on the Luxd repository, bumping the coreth dependency to use this release candidate. Once proven stable, an Luxd release should be created, after which you can create a coreth release.

### Release

If a successful release candidate was created and integrated in a release of Luxd, you can now create a release.
...
1. Create a new release on Github, either using:
    - the [Github web interface](https://github.com/luxfi/subnet-evm/releases/new)
        ...
        1. Set the description using this format:
            ...
            ```markdown
            This is the Coreth version used in Luxd@v1.13.1
    ...
    - the Github CLI `gh`:
        ...
        NOTES="This is the Coreth version used in Luxd@v1.13.1
...
Note this release will likely never be used in Luxd, which should always be using release candidates to accelerate the development process. However it is still useful to have a release to indicate the last stable version of coreth.

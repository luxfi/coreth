...
> Note: `snowman-api-enabled` is the legacy luxd config key name. Current luxd builds also accept `consensus-api-enabled` as the canonical alias; both refer to the same toggle.

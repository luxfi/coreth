...
Now that LuxGo depends on the local version of Coreth, we can build with the normal build script:
...
Note: the C-Chain originally ran in a separate process from the main LuxGo process and communicated with it over a local gRPC connection. When this was the case, LuxGo's build script would download Coreth, compile it, and place the binary into the `node/build/plugins` directory.
